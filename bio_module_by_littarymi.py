...
Ваша вторая рука в биовойнах)
    ...
    strings = {
        ...
        "not_reply": "<emoji document_id=5404553572727660202>❌</emoji> Нема реплая.",
        
        "not_args": "<emoji document_id=5404553572727660202>❌</emoji> Нема аргументов.",
        
        "nolink": "<emoji document_id=5197248832928227386>😢</emoji> Нема линка.",

        "hueta": "🤔 Что за хуета?",
        
        "r.save":   
            "<emoji document_id=6037300193567706158>💘</emoji> Мавпа <b><code>{}</code></b> в зарлисте.\n"
            "<b>☣️ +{}{}</b> био-опыта.",
        "auto.save":   
            "<emoji document_id=6037300193567706158>💘</emoji> Мавпа <b><code>{}</code></b> в зарлисте.\n"
            "<b>☣️ {}+{}</b> био-опыта.",        
        "search":
            "<emoji document_id=5404861942789579369>✅</emoji> Мавпа <code>{}</code> приносит:\n"
            "<b>☣️ +{} био-опыта.</b>\n"
            "📆 Дата: <i>{}</i>",
        
        "nf": "<emoji document_id=5319055531371930585>🙅‍♂️</emoji> Мавпа не найдена, бей иво.",
        
        "no_user": "<emoji document_id=5404553572727660202>❌</emoji> user {} don't exist.",

        "nous": "<emoji document_id=5451627230335606973>😔</emoji> Мавпа не существует.",
        ...
        "user_rm": "<emoji document_id=5404553572727660202>❌</emoji> Помощник <b><code>{}</code></b> удалён, туди його.",
        
        "user_add": "<emoji document_id=5404861942789579369>✅</emoji> Помощник <b><code>{}</code></b> добавлен!",
...
Получает айди пользователя.
        ...
        if not reply:
            ...
            if not user:
                return await message.reply("Пробуй заново")
...
(без них бьет по ответу с 10 патов)
        ...
        if not args:
            ...
            hui = f'<code>/заразить 5 @{vlad}<code>\nну ты и олух <emoji document_id=6037300193567706158>💘</emoji>'
        ...
        if self.config["Доступ к зарлисту"] == True:
            ...
            if re.search(r"био", text, flags=re.ASCII):
                ...
                if "🔬 ТОП ЛАБОРАТОРИЙ БЕСЕДЫ" in a:
                    sms += "<emoji document_id=5377554536426644484>🔵</emoji> еда чата:\n"
...
Используй ответом на биотопы/жертвы и т.п
        ...
        hiunya = [
            f"{emoji}<b>ща бомба буде...</b> {utils.ascii_face()}",
...
#######################   
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "Доступ к заражениям",                                    #нахуя ты это читаешь?
                True,
